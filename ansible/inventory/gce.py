...
class GceInventory(object):
    ...
    def parse_env_zones(self):
        ...
        import csv
        reader = csv.reader(
            [os.environ.get('GCE_ZONE', "")], skipinitialspace=True)
    ...
    def node_to_dict(self, inst):
        ...
        if 'subnetwork' in inst.extra['networkInterfaces'][0]:
            subnet = inst.extra['networkInterfaces'][0]['subnetwork'].split(
                '/')[-1]
        # default to exernal IP unless user has specified they prefer internal
        if self.ip_type == 'internal':
            ssh_host = inst.private_ips[0]
        else:
            ssh_host = inst.public_ips[0] if len(
                inst.public_ips) >= 1 else inst.private_ips[0]
